...
if [ $# -lt 1 ]; then
  ...
  echo 'shell - Starts up a bash shell in the container.'
  exit
fi
...
rc)  echo "Starting Console in Docker Container.."
    ...
    ;;
rdbm)  echo  "Running rake db:create db:migrate in Docker container.."
    vagrant ssh -c "sh /app/docker/scripts/rdbm.sh"
    ;;
shell) echo "Starting up a bash shell in Docker Container..."
    $HOME/apps/butler/docker/scripts/shell.sh
    ;;
...
cmd) echo "running '$2' in docker container in /app"
    vagrant ssh -c "sh /app/docker/scripts/cmd.sh '$2'"
    ;;
